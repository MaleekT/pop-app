...
import { cardStyle, ctaStyle, outcomeColor, formatMarketTitle, friendlyTxError } from '@/components/predict/ui'
...
export interface ParlayDetailProps {
  id: string
  // The section this view is mounted under. Drives Back on a cold load, so a ticket opened from
  // Activity returns to Activity rather than the Parlay builder.
  backHref: string
}

// The parlay ticket view, mounted under BOTH sections: /parlay/[id] for the Parlay builder and
// /activity/parlay/[id] for a wallet's own tickets. Each section owning its own URL is what keeps
// AppNav's pathname check honest: /parlay/* deliberately lights Predict (AppNav.tsx:18), so a ticket
// opened from Activity necessarily left Activity until it had a URL of its own.
export function ParlayDetail({ id, backHref }: ParlayDetailProps) {
